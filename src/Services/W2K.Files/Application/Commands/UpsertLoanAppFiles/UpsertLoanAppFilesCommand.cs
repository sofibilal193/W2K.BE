...
public readonly record struct UpsertLoanAppFilesCommand(int OfficeId, int LoanAppId, IReadOnlyCollection<LoanAppFileCommand> Files) : IRequest<IReadOnlyCollection<FileDto>>;

public readonly record struct LoanAppFileCommand(string FileName, string ContentType, byte[] Content);

public class UpsertLoanAppFilesCommandValidator : AbstractValidator<UpsertLoanAppFilesCommand>
{
    public UpsertLoanAppFilesCommandValidator()
    {
        RuleFor(x => x.OfficeId)
            .GreaterThan(0);

        RuleFor(x => x.LoanAppId)
            .GreaterThan(0);
